...
public class GUI extends JFrame {
    ...
    private final JButton exportButton = new JButton("Export (CSV+PDF)");
    private final AnalyticsService analyticsService = new AnalyticsService();

    private final JButton chartButton = new JButton("Chart");
    private JDialog chartDialog;
    private final VehicleCountSeries vehicleSeries =
            new VehicleCountSeries(600); // z.B. letzte ~2 Minuten
    private final VehicleChartPanel chartPanel = new VehicleChartPanel();
    ...
    private void loadTlsDurationsFromNet() {
        try {
            String netPath = "/Users/alikandas/Downloads/hamzamap.net.xml";
    ...
    private void openChartWindow() {
        ...
        if (chartDialog != null && chartDialog.isVisible()) {
            chartDialog.toFront();
            return;
        }

        chartDialog = new JDialog(this, "Live Vehicle Chart", false);
    ...
    private void initLayout() {
        ...
        cfgField.setText("/Users/alikandas/Downloads/config.sumocfg");
        ...
        gbc.gridx=3; topPanel.add(chartButton, gbc);

        gbc.gridy=5; gbc.gridx=0; gbc.gridwidth=4;
        topPanel.add(statusLabel, gbc);

        gbc.gridy=5; gbc.gridx=1; gbc.gridwidth=3;
        ...
        chartButton.addActionListener(e -> openChartWindow());
    ...
    private void onExportClicked(ActionEvent e) {
        try {
            ...
            // 1) Snapshot holen
            var snapshot = controller.getDataGathering().getStore().snapshot();
            System.out.println("[EXPORT] datapoints = " + snapshot.size());

            // 2) vehiclesEver korrekt (unique vehicleIds aus Snapshot)
            int vehiclesEver = vehiclesEverFromSnapshot(snapshot);
            System.out.println("[EXPORT] vehiclesEver = " + vehiclesEver);

            // 3) Dateinamen + absolute Pfade
            String ts = String.valueOf(System.currentTimeMillis());
            String baseDir = System.getProperty("user.dir");
            String csv = baseDir + "/export_" + ts + ".csv";
            String pdf = baseDir + "/export_" + ts + ".pdf";

            System.out.println("[EXPORT] csv path = " + csv);
            System.out.println("[EXPORT] pdf path = " + pdf);

            // 4) CSV export
            new CsvExporter().export(csv, snapshot);
            System.out.println("[EXPORT] CSV export successful.");

            // 5) PDF export (Chart als Bild)
            System.out.println("[EXPORT] starting PDF export...");
            java.awt.image.BufferedImage chartImg = chartPanel.createImage();
            System.out.println("[EXPORT] chart image = " + chartImg.getWidth() + "x" + chartImg.getHeight());
    ...
    private int vehiclesEverFromSnapshot(java.util.List<DataPoint> snapshot) {
    ...
    private void updateFromSumo() throws Exception {
        ...
        int vehiclesNow = controller.getVehicleIds().size();
        vehicleSeries.add(System.currentTimeMillis(), vehiclesNow);
        chartPanel.setValues(vehicleSeries.values());
